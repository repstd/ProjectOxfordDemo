...
public class FaceServiceClient implements IFaceServiceClient {
    ...
    @Override
    public Face[] detect(String url, boolean analyzesFaceLandmarks, boolean analyzesAge, boolean analyzesGender, boolean analyzesHeadPose) throws ClientException {
    ...
    public Face[] detect(InputStream image, boolean analyzesFaceLandmarks, boolean analyzesAge, boolean analyzesGender, boolean analyzesHeadPose) throws ClientException, IOException {
        ...
        Log.d("POST RESULT ",json);
        Type listType = new TypeToken<List<Face>>() {
        }.getType();
        List<Face> faces = this.gson.fromJson(json, listType);
        Log.d("POSTED FACE ",Integer.toString(faces.size()));
        return faces.toArray(new Face[faces.size()]);
    }
